...
import { InsuranceIndustryPage } from "@/components/sections/insurance/InsuranceIndustryPage";
...
export async function generateMetadata(): Promise<Metadata> {
  return getCmsPageMetadata("insurance");
}

export default async function InsurancePage() {
  const content = await getCmsDocument("insurance");
  ...
  return (
    <main className="relative min-h-screen bg-[#f4f4f5] text-[#18181b]">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 z-0 h-[822px] overflow-hidden">
        <Image src="/insurance/bg-top.webp" alt="" fill sizes="100vw" className="object-cover object-top" priority />
      </div>

      <div className="relative z-10 px-5">
        <Navbar />
        <InsuranceIndustryPage content={content} />
        ...
        <div className="relative z-0 -mx-5 pt-[120px]">
          <div aria-hidden className="pointer-events-none absolute inset-x-0 bottom-0 z-0 h-[822px] overflow-hidden">
            <Image src="/insurance/bg-bottom.webp" alt="" fill sizes="100vw" className="object-cover object-bottom" />
          </div>
          <PricingCTABanner />
          <Footer />
        </div>
